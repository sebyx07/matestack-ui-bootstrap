...
describe 'Bootstrap::Components::Alert', type: :feature, js: true do
  ...
  xit 'should create a simple primary alert without any content' do
    matestack_render { bs_alert }
    visit example_path
    expect(page).to have_selector('div.alert.alert-primary[role=alert]')
  end

  xit 'can get a title with various sizes and some content' do
  ...
  xit 'can render a block for custom content' do
  ...
  xit 'gets correct classes for animation' do
  ...
  xit 'can receive custom css classes' do
  ...
  xit 'can be closed per event' do
    matestack_render {
  ...
  xit 'can be disposed per event' do
